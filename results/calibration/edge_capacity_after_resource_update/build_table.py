...
def main():
    ...
    print(f"{'tgtRPS':>6} {'obsRPS':>7} {'p95ms':>8} {'p90ms':>8} {'maxms':>9} {'cpu%':>6} {'fail':>6} {'reqs':>6}")
    for r in levels:
        cpu_pct = r["cpu_pct"] if r["cpu_pct"] != "" else "-"
        print(f"{r['target_rps']:>6} {r['observed_rps']:>7.1f} {r['p95_ms']:>8.1f} "
              f"{r['p90_ms']:>8.1f} {r['max_ms']:>9.1f} {str(cpu_pct):>6} "
